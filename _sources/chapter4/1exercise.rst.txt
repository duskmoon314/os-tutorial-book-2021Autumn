...
.. tabs::
    ...
    .. group-tab:: C
        ...
        - 我们不会在以后的实验中用到优先级调度，而 ch3 实现的内存检查也会被虚存直接覆盖。所以你可以直接使用基本框架代码继续实验，无需 merge ch3 的修改。
        - ``make test BASE=1``
...
.. hint::
    ...
    - (C) 匿名映射的页可以使用 kalloc() 得到。
    - (C) 注意 kalloc 不支持连续物理内存分配，所以你必须把多个页的 mmap 逐页进行映射。
...
- 注意目录要求，报告命名 ``lab2.md`` 或 ``lab2.pdf``，位于 ``reports`` 目录下。命名错误视作没有提交。不需要删除 ``lab1.md/pdf``。后续实验同理。
- 完成 ch4 问答作业。
